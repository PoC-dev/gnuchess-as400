/*
 * GNU Chess 5.0 - ttable.c - transposition table code Copyright (c) 1999-2002
...
void TTPut(int side, int depth, int ply, int alpha, int beta, int score,
    int move)
...
int TTGet(int side, int depth, int ply, int alpha
    __attribute__((unused)), int beta
    __attribute__((unused)), int *score, int *move)
 ...
 *****************************************************************************/
{
    ...
    if (MATESCORE(s))
        s -= (s > 0 ? ply : -ply);
    if (Key == t->key && ((ply & 1 && score == s) || (!(ply & 1)
                && score == -s))) {
        *move = t->move;
        return (1);
    }
