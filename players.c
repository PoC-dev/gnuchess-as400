...
/*
 * GNU Chess 5.0 - player.c - database on players Copyright (c) 1999-2002 Free
...
static int scorecompare(const void *aa, const void *bb) {
    ...
    ascore =
        100 * (a->wins + (a->draws / 2)) / (a->wins + a->draws + a->losses);
    bscore =
        100 * (b->wins + (b->draws / 2)) / (b->wins + b->draws + b->losses);
    if (bscore > ascore)
        return (1);
    else if (bscore < ascore)
        return (-1);
...
static int namecompare(const void *aa, const void *bb) {
    ...
    if (strcmp(a->player, b->player) > 0)
        return (1);
    else if (strcmp(a->player, b->player) < 0)
        return (-1);
    else
        return (0);
}
...
void DBListPlayer(const char *style) {
    ...
    for (i = 0; i < totalplayers; i++) {
        printf("%s %2.0f%% %d %d %d\n", playerdb[i].player,
            100.0 * (playerdb[i].wins +
                ((float)playerdb[i].draws / 2)) / (playerdb[i].wins +
                playerdb[i].draws + playerdb[i].losses), playerdb[i].wins,
            playerdb[i].losses, playerdb[i].draws);
        if ((i + 1) % 10 == 0) {
    ...
    if ((wfp = fopen(PLAYERFILE, "w")) != NULL) {
        for (i = 0; i < totalplayers; i++) {
            result1 =
                100.0 * (playerdb[i].wins +
                ((float)playerdb[i].draws / 2)) / (playerdb[i].wins +
                playerdb[i].draws + playerdb[i].losses), result2 = (int)result1;
            fprintf(wfp, "%s %d %d %d\n", playerdb[i].player, playerdb[i].wins,
                playerdb[i].losses, playerdb[i].draws);
        }
    }
    fclose(wfp);
    ...
    if ((rfp = fopen(PLAYERFILE, "r")) != NULL) {
        while (!feof(rfp)) {
            n = fscanf(rfp, "%s %d %d %d\n", playerdb[totalplayers].player,
                &playerdb[totalplayers].wins, &playerdb[totalplayers].losses,
...
int DBSearchPlayer(const char *player) {
    ...
    for (i = 0; i < totalplayers; i++)
        if (strncmp(playerdb[i].player, player,
                strlen(playerdb[i].player)) == 0) {
            index = i;
            break;
        }
    return (index);
