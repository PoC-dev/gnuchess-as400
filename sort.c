/*
 * GNU Chess 5.0 - sort.c - move sorting code Copyright (c) 1999-2002 Free
 ...
 *****************************************************************************/
{
    ...
    for (p = TreePtr[ply]; p < TreePtr[ply + 1]; p++) {
        ...
        p->score +=
            history[side][(p->move & 0x0FFF)] + taxicab[f][D5] - taxicab[t][E4];
    ...
    switch (pickphase[ply]) {
        ...
        case PICKGEN1:
            ...
            for (p2 = p[ply]; p2 < TreePtr[ply + 1]; p2++)
                p2->score =
                    SwapOff(p2->move) * WEIGHT + Value[cboard[TOSQ(p2->move)]];
