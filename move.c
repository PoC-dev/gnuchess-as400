/*
 * GNU Chess 5.0 - move.c - make and unmake moves code Copyright (c) 1999-2002
 ...
 **************************************************************************/
{
    ...
    ************************************************************************/
    ...
    if (mvstr[strlen(mvstr) - 1] == '+' || mvstr[strlen(mvstr) - 1] == '#'
        || mvstr[strlen(mvstr) - 1] == '=')
        mvstr[strlen(mvstr) - 1] = '\000';
 ...
 *****************************************************************************/
{
    ...
    if (piece == pawn) {
        ...
        if (side == white) {
            if (!(MoveArray[pawn][f] & BitPosArray[t] & enemy) && !(t - f == 8
                    && cboard[t] == empty) && !(t - f == 16 && RANK(f) == 1
                    && !(FromToRay[f][t] & blocker)))
                return (false);
        } else if (side == black) {
            if (!(MoveArray[bpawn][f] & BitPosArray[t] & enemy) && !(t - f == -8
                    && cboard[t] == empty) && !(t - f == -16 && RANK(f) == 6
                    && !(FromToRay[f][t] & blocker)))
                return (false);
        }
    }
     ...
     */
        ...
        if (side == white) {
            if (!(MoveArray[piece][f] & BitPosArray[t]) && !(f == E1 && t == G1
                    && board.flag & WKINGCASTLE
                    && !(FromToRay[E1][G1] & blocker) && !SqAtakd(E1, black)
                    && !SqAtakd(F1, black)) && !(f == E1 && t == C1
                    && board.flag & WQUEENCASTLE
        ...
        if (side == black) {
            if (!(MoveArray[piece][f] & BitPosArray[t]) && !(f == E8 && t == G8
                    && board.flag & BKINGCASTLE
                    && !(FromToRay[E8][G8] & blocker) && !SqAtakd(E8, white)
                    && !SqAtakd(F8, white)) && !(f == E8 && t == C8
                    && board.flag & BQUEENCASTLE
