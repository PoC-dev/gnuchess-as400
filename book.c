/*
 * GNU Chess 5.0 - book.c - book code Copyright (c) 1999-2002 Free Software
 * Foundation, Inc.
 *
...
static int check_magic(FILE * f) {
    ...
    return (r == MAGIC_LENGTH && memcmp(buf, magic_str, MAGIC_LENGTH) == 0);
}
...
static int is_empty(uint32_t index) {
    return bookpos[index].key == 0 && bookpos[index].wins == 0
        && bookpos[index].draws == 0 && bookpos[index].losses == 0;
}
...
static void buf_to_book(void) {
     ...
     */
    for (DIGEST_START(i, key); DIGEST_COLLISION(i, key); DIGEST_NEXT(i, key))
        /*
         * Skip
         */
        bookhashcollisions++;
...
int BookQuery(int BKquery) {
    ...
    for (i = 0; i < icnt; i++) {
        for (DIGEST_START(j, posshash[i]); !DIGEST_EMPTY(j);
            ...
            if (DIGEST_MATCH(j, posshash[i])) {
                ...
                if (!found) {
                    ...
                    pref[mcnt].score = m[i].score =
                        100 * (r[i].wins + (r[i].draws / 2)) / (MAX(r[i].wins +
                            r[i].losses + r[i].draws, 1)) + r[i].wins / 2;
                ...
                break;                   /* If we found a match, the following
                                          * positions can not match */
            }
        }
    }
    ...
    if (mcnt + 1) {
        if (!(flags & XBOARD) || BKquery == 1) {
            for (i = 0; i <= mcnt; i++) {
                if (!(flags & XBOARD) || BKquery == 1) {
                    ...
                    tot =
                        r[matches[i]].wins + r[matches[i]].draws +
                        r[matches[i]].losses;
                    fprintf(ofp, " %s(%2.0f/%d/%d/%d) ", SANmv,
                        100.0 * (r[matches[i]].wins +
                            (r[matches[i]].draws / 2.0)) / tot,
                        (int)r[matches[i]].wins, (int)r[matches[i]].losses,
        ...
        if (bookmode == BOOKRAND) {
            ...
            if (!(flags & XBOARD)) {
                ...
                tot =
                    r[matches[k]].wins + r[matches[k]].draws +
                    r[matches[k]].losses;
                if (tot != 0)
                    printf("B p=%2.0f\n",
                        100.0 * (r[matches[k]].wins +
                            r[matches[k]].draws) / tot);
                else
