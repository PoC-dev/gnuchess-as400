/*
 * GNU Chess 5.0 - common.h - common symbolic definitions Copyright (c)
...
#define	DRAWSCORE  (computerplays == board.side ? (opprating - myrating) / 4 : \
(myrating - opprating ) / 4)
...
/*
 * Smaller HASHSLOT defaults 20011017 to improve blitz play and make it easier
...
enum {
    BOOK_SUCCESS,
    BOOK_EFORMAT,                        /* Wrong format (e.g. produced by
                                          * older version) */
    BOOK_EMIDGAME,                       /* Move is past the opening book's
                                          * move limit */
    BOOK_EIO,                            /* I/O error, e.g. caused by wrong
                                          * permissions */
    BOOK_EFULL,                          /* Book hash is full, new position was
                                          * not added. */
    BOOK_ENOBOOK,                        /* No book present */
    BOOK_ENOMOVES,                       /* No moves found (in BookQuery()
                                          * only) */
