/*
 * GNU Chess 5.0 - search.c - tree-search code Copyright (c) 1999-2002 Free
...
void ShowThinking(leaf * p, int ply) {
    ...
    fprintf(stderr, "\r%2d.         %2d/%2d%10s    ", Idepth / DEPTH,
        (int)(p - TreePtr[ply] + 1), (int)(TreePtr[ply + 1] - TreePtr[ply]),
        SANmv);
    fflush(stderr);
}
 ...
 *************************************************************************/
    ...
    if (InChk[ply] && /* ChkCnt[ply] < Idepth-1 */ ply <= 2 * Idepth / DEPTH) {
        ...
        depth += DEPTH;
        extend = true;
    } else if (!KingThrt[side][ply - 1] && KingThrt[side][ply]
        && ply <= 2 * Idepth / DEPTH) {
        KingExtCnt++;
        extend = true;
        depth += DEPTH;
     ...
     */
        ...
        && (board.material[computer] - board.material[1 ^ computer] ==
            RootMaterial)) {
        RcpExtCnt++;
        depth += DEPTH;
        extend = true;
 ...
 *****************************************************************************/
    ...
    if (flags & USENULL && g0 != NULLMOVE && depth > DEPTH && nodetype != PV
        && !InChk[ply] && MATERIAL + ValueP > beta && beta > -MATE + ply
        && donull && board.pmaterial[side] > ValueB && !threatply) {
        TreePtr[ply + 1] = TreePtr[ply];
        MakeNullMove(side);
        nullscore =
            -Search(ply + 1, depth - DEPTH - R, -beta, -beta + 1, nodetype);
        UnmakeNullMove(xside);
        if (nullscore >= beta) {
            NullCutCnt++;
            return (nullscore);
        }
        if (depth - DEPTH - R >= 1 && MATERIAL > beta
            && nullscore <= -MATE + 256) {
            depth += DEPTH;
            extend = true;
        }
 ...
 *************************************************************************/
    ...
    fcut = (!extend && nodetype != PV && depth == 2 * DEPTH
        && FUTSCORE <= alpha);
    if (!fcut) {
    ...
    NodeCnt++;
    g0 = g1 = 0;
    while ((g0 = SqAtakd(board.king[side], xside)) > 0 || (fcut
            && FUTSCORE < alpha && !SqAtakd(board.king[xside], side)
            && !MateScan(xside))) {
        if (g0 == 0)
            g1++;
        UnmakeMove(xside, &p->move);
 ...
 *****************************************************************************/
            ...
            score =
                -Search(ply + 1, depth - DEPTH, -alpha - 1, -alpha, nodetype);
            if (score > best) {
                if (savenode == PV)
                    nodetype = PV;
                if (alpha < score && score < beta) {
                    score =
                        -Search(ply + 1, depth - DEPTH, -beta, -score,
                        nodetype);
                }
                if (nodetype == PV && score <= alpha
                    && Game[GameCnt + 1].move == NULLMOVE) {
                    score =
                        -Search(ply + 1, depth - DEPTH, -alpha, INFINITY,
                        nodetype);
                }
            }
        }
        ...
        if ((score >= beta || nodetype == PV) && !InChk[ply] && g0 != NULLMOVE
            ...
            if (!nullthreatdone) {
                ...
                MakeNullMove(side);
                nullscore =
                    -Search(ply + 1, depth - 1 - R, -alpha + THREATMARGIN,
    ...
    if (flags & XBOARD) {
        if (score > MATE - 255) {
            printf("%d%c Mat%d %d %lu\t", Idepth / DEPTH, c,
                (int)(MATE + 2 - abs(score)) / 2, (int)(et * 100),
                NodeCnt + QuiesCnt);
            if (ofp != stdout)
                fprintf(ofp, "%2d%c%7.2f  Mat%02d%10lu\t", Idepth / DEPTH, c,
                    et, (MATE + 2 - abs(score)) / 2, NodeCnt + QuiesCnt);
            ...
            if (ofp != stdout)
                fprintf(ofp, "%2d%c%7.2f%7d%10lu\t", Idepth / DEPTH, c, et,
        ...
        if (score > MATE - 255) {
            ...
            if (ofp != stdout)
                fprintf(ofp, "\r%2d%c%7.2f  Mat%02d%10lu\t", Idepth / DEPTH, c,
                    et, (MATE + 2 - abs(score)) / 2, NodeCnt + QuiesCnt);
        } else if (score < -MATE + 255) {
            printf("\r%2d%c%7.2f -Mat%02d%10lu\t", Idepth / DEPTH, c, et,
                (MATE + 2 - abs(score)) / 2, NodeCnt + QuiesCnt);
            ...
            printf("\r%2d%c%7.2f%7d%10lu\t", Idepth / DEPTH, c, et, score,
                NodeCnt + QuiesCnt);
            if (ofp != stdout)
                fprintf(ofp, "\r%2d%c%7.2f%7d%10lu\t", Idepth / DEPTH, c, et,
                    score, NodeCnt + QuiesCnt);
        }
    }
