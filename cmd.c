/*
 * GNU Chess 5.0 - cmd.c - command parser Copyright (c) 1999-2002 Free Software
 * Foundation, Inc.
 *
...
static char cmd[INPUT_SIZE], file[INPUT_SIZE], s[INPUT_SIZE],
    logfile[INPUT_SIZE], gamefile[INPUT_SIZE], userinput[INPUT_SIZE];
 ...
 *************************************************************************/
{
    ...
    if (isatty(STDIN_FILENO)) {
        sprintf(s, "%s (%d) %c ", color[board.side], (GameCnt + 1) / 2 + 1,
            prompt);
        inputstr = readline(s);
        if (inputstr == NULL)
            return;
    ...
    if (strcmp(cmd, "quit") == 0 || strcmp(cmd, "exit") == 0)
        ...
        if (flags & XBOARD) {
            /*
             * Note: change this if "analyze" or "draw" commands are added,
             * etc.
             */
        ...
        /*
         * If ping is received when we are on move, we are supposed to reply
         * only after moving.  In this version of GNU Chess, we never read
         * commands while we are on move, so we don't have to worry about that
         * here.
         */
        printf("pong %s\n", inputstr);
        fflush(stdout);
 ...
 ************************************************************************/
{
    ...
    if (strcmp(cmd, "board") == 0)
        ...
        wk = ScoreK(white);
        bk = ScoreK(black);
        printf("White:  Mat:%4d/%4d  P:%d  N:%d  B:%d  R:%d  Q:%d  K:%d  \
Dev:%d  h:%d x:%d\n",
            board.pmaterial[white], board.material[white], wp, ScoreN(white),
            ScoreB(white), ScoreR(white), ScoreQ(white), wk, ScoreDev(white),
            hunged[white], ExchCnt[white]);
        printf("Black:  Mat:%4d/%4d  P:%d  N:%d  B:%d  R:%d  Q:%d  K:%d  \
Dev:%d  h:%d x:%d\n",
        board.pmaterial[black], board.material[black], bp, ScoreN(black),
            ScoreB(black), ScoreR(black), ScoreQ(black), bk, ScoreDev(black),
            hunged[black], ExchCnt[black]);
